...
import {
  ...
  ChevronLeft, Zap, LogOut, CreditCard, Coins, Trophy, Smartphone, Diamond, Crown, Gift,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile overlay */}
      {sidebarOpen && (
        <div className="fixed inset-0 z-20 bg-black/50 lg:hidden" onClick={() => useUIStore.getState().setSidebarOpen(false)} />
      )}
      ...
      <aside className={cn(
        ...
        <div className="flex items-center justify-between px-4 py-5 border-b border-gray-800">
          ...
          <button onClick={toggleSidebar} className={cn('p-1 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition', !sidebarOpen && 'hidden lg:flex')}>
        ...
        <nav className="flex-1 overflow-y-auto py-3 scrollbar-thin">
          {nav.map((item, i) => {
            if ('section' in item) {
              return sidebarOpen ? (
                <p key={i} className="px-4 pt-4 pb-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">{item.section}</p>
        ...
        <div className="border-t border-gray-800 p-3">
          {sidebarOpen ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={logout} className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-gray-800 transition">
                <LogOut className="w-4 h-4" />
              </button>
            </div>
          ) : (
            <button onClick={logout} className="flex justify-center w-full p-2 text-gray-400 hover:text-red-400 hover:bg-gray-800 rounded-lg transition">
